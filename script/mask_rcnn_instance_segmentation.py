...
class MaskRCNNInstanceSegmentation(ConnectionBasedTransport):
    ...
    def callback(self, imgmsg):
        ...
        for ins_id, (mask, label) in enumerate(zip(masks, labels)):
            ...
            print("self.fg_class_names = {}".format(self.fg_class_names["fg_class_names"][label]))
            # class_name = self.fg_class_names[label]
            class_name = self.fg_class_names["fg_class_names"][label]
        ...
        cls_msg = ClassificationResult(
            ...
            labels=labels,
            label_names=[self.fg_class_names["fg_class_names"][l] for l in labels],
            label_proba=scores,
        )
        ...
        if self.pub_viz.get_num_connections() > 0:
            n_fg_class = len(self.fg_class_names)
            captions = ['{:d}: {:s}'.format(l, self.fg_class_names["fg_class_names"][l])
                        for l in labels]
